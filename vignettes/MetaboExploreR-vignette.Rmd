...
Before installing `MStargetR`, you need to have Docker Desktop installed on your system. You can download it from the official Docker website: <https://www.docker.com/get-started/>

Once Docker is installed and running, you can install `MStargetR` from GitHub using the following commands in R:
...
The `MStargetR` workflow consists of four main steps. We will use the example data included in the package to demonstrate the workflow.
...
The MRM transition list is a tab-separated or csv file that contains information about the molecules to be quantified. An example file, `LGW_lipid_mrm_template_v1.tsv`, is included in the package. Let's inspect its contents.
...
1. `transition_checkR` 
Checks Q1 and Q3 transitions to ensure all transitions are unique. Please use the below example to test it out!
...
Checks if all internal standards from the `Note` column in the transition list has a match in `SIL_name` of the concentration guide. Please use the below example to test it out!
...
Previously, we demonstrated how to set up MStargetR for a single method. However, the package also supports multi method analysis provided that a consistent long-term reference material (LTR) has been used across all methods 
To enable this functionality, users must supply the appropriate transition lists and concentration guides for each method.
